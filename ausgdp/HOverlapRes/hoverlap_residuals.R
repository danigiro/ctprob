...
dir.create(file.path(".","HOverlapRes",model, trans), recursive = TRUE, showWarnings = FALSE)
...
for(j in 0:(end_traing-1)){
    ...
    for(i in 1:NCOL(DATA)){
      ...
      if(k == 1){
        listResOver[[idK]][[idS]] <- as.vector(t(sapply(1:(H/k), function(x){
          if(trans == "log"){
            xlog <- listFit[[idK]][[idS]]$x
            xlog[xlog<=0] <- min(xlog[xlog>0])/2
            listFit[[idK]][[idS]]$x <- xlog
          }
          tmp_res <- ts_train-fitted(listFit[[idK]][[idS]], h = x)
        ...
        res_out <- as.vector(do.call(rbind, lapply(1:NROW(opt), function(idr){
          ...
          t(sapply(1:(H/k), function(x){
            ...
            tmp_res[!idna] <- true_val - fitted(mod, h = x)
            out <- rep(NA, length(tmp_res))
            id <- x:(length(tmp_res)-((H/k)-x))
            out[1:length(id)] <- tmp_res[id]
            out
  ...
  save(listRes,
       file = file.path(".","HOverlapRes",model, trans, itername))
}
rm(list = ls(all = TRUE))