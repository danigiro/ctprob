...
sel_prob <- c("ctsam", "bsam", "hsam", "hbsam", "ctsamh", "bsamh", "hsamh", "hbsamh", "ctjb")
...
data |>
  ...
  kbl(format = "latex", digits = 3, booktabs = TRUE, 
      linesep = "",
      align = "cccccc", position = "t",
      col.names = c("\\multirow{-5}{*}{\\parbox{2cm}{\\centering\\textbf{Reconciliation\\\\approach}}}", 
                    "G", "B", "H", "HB", "G", "B", "H", "HB", "\\multirow{-4}{*}{Bootstrap}"),
      escape = FALSE) |>
  add_header_above(c("", "In-sample residuals" = 4, "Multi-step residuals" = 4, ""), 
                   escape = TRUE, line_sep = 0, line = FALSE) |>
  add_header_above(c("", "Gaussian frameworks: sample covariance matrix" = 8, ""), 
                   escape = TRUE, line_sep = 0, line = FALSE) |>
  ...
  save_kable(paste0("./Tables/norm_cov.tex"), self_contained = FALSE)
...
rbind(crps_0, crps_k) |>
  ...
  add_header_above(c("", "In-sample residuals" = 4, "Multi-step residuals" = 4, ""), 
                   escape = TRUE, line_sep = 0, line = FALSE) |>
  add_header_above(c("", "Gaussian frameworks: shrinkage covariance matrix" = 8, ""), 
                   escape = TRUE, line_sep = 0, line = FALSE) |>
  add_header_above(c("", "Base forecasts' sample approach" = 9), 
                   escape = TRUE, bold = TRUE, line_sep = 0) |>
  column_spec(1, width = "2.5cm", latex_valign = "m") |>
