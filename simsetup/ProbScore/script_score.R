...
scores_file <- list.files(file.path(".","ProbScore", "scores"), full.names = TRUE)
df_es_mean <- NULL
df_crps_mean <- NULL
df_mis_mean <- NULL
...
for(i in 1:length(scores_file)){
  ...
  df_es_mean <- rbind(df_es_mean, 
                      df_es |>
                        ...
                        summarise(sd = sd(value)/sqrt(length(value)),
                                  value = mean(value), .groups = "drop") |>
                        separate(name, into = c("meth", "comb", "nn"), sep = "-"),
                      df_es |>
                        pivot_longer(-c(prob, serie, ite, k, h)) |>
...
save(df_crps_mean, df_es_mean, df_mis_mean, 
     file = file.path(".","ProbScore", paste("scores.RData", sep = "_")))
