...
for(j in 1:length(listFiles)){
  ...
  for(comb in listComb){
    bts_free <- matrix(NA, ncol = NCOL(base), nrow = NCOL(C))
    free <- matrix(NA, nrow = NROW(base), ncol = NCOL(base))
    
    for(i in 1:NCOL(C)){
      bts_free[i, ] <- thfrec(basef = base[NROW(C)+i, ], m = m, comb = comb, res = res[NROW(C)+i, ], keep = "recf")
    }
    
    free <- ctbu(bts_free[,-c(1:(ks*B))], C = C, m = m)
