...
\node[right= -0.75em of e1.north east, yshift = 2em,
       anchor=north west, font = {\fontsize{9}{10}\selectfont}] {$\xrightarrow[\text{residuals}]{\text{bootstrapped}}$};
...
\end{tikzpicture}
	\caption{Example of bootstrapped residuals for 3 linearly constrained quarterly time series (see \autoref{fig:hierS}). On the left there are the residual matrices with 4 years of data ($N=4$): the green, blue, red and black colors correspond, respectively, to years 1, 2, 3 and 4. On the right the bootstrapped residuals are represented.}
	\label{fig:res_boot}
