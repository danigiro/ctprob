...
\node[draw, thick, circle, inner sep=1mm, text width=0.5cm, text centered, minimum width = 0.5cm, minimum height = 0.5cm, below = 1.5cm of cov, label={[align=center, font=\footnotesize]left:{Covariance approx., \\for point R.F.}}] (covapprx) {$\Omegavet_{ct}$};
...
\draw [-{Triangle[scale=1.5]}, decorate, decoration={snake,pre length=4pt,post length=15pt}, thick, shorten >= 5pt] (dist) -- (sim_reco)  node[draw=none, midway, font = \footnotesize, text width = 6.25cm, text centered, fill = white, yshift = 0.2cm] {HF-BTS\\[0.2cm]$N\left(\widetilde{\textbf{b}}^{[1]}, \widetilde{\Omegavet}_{hf-bts} \right)$, $\widetilde{\xvet}_{\ell}=\textbf{S}_{ct} \widetilde{\textbf{b}}^{[1]}_{\ell}$} ;
\draw[-{Triangle[scale=1]}, decorate, thick] (covapprx.north east) -- (dist.south west);
\end{tikzpicture}
	\endgroup
	\caption{Visual description of cross-temporal forecast reconciliation in the Gaussian framework, as described in Section \ref{ssec:prob_pf}. The acronyms R.F and B.F. stand for Reconciled and Base Forecasts, respectively. HF-BTS stands for High Frequency Bottom Time Series.}
	\label{fig:gaussrel}
