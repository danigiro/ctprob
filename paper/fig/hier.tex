...
\centering
		\begin{tikzpicture}[baseline=(current  bounding  box.center),
			...
			\relation{0.2}{B}{T};
		\end{tikzpicture}
		\caption{\textbf{(a)} A simple two-level hierarchical structure for 3 linearly constrained time series with $n=3$, $n_a = 1$ and $n_b = 2$. \textbf{(b)} Hierarchical structure for a quarterly series ($m = 4$ and $\mathcal{K} = \{4,2,1\}$).}
\label{fig:hierS}
\end{figure}