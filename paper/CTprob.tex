...
% \autoref{tab:cov_app} presents some approximations for the cross-sectional and the temporal covariance matrices. \cite{nystrup2020} and \cite{difonzo2023} provide further temporal reconciliation approaches that exploit possible information in the residuals' autocorrelation.
...
$$
	e_{i,h,t}^{[k]} = x_{i,t}^{[k]} - \widehat{x}_{i,t+h|t}^{[k]}\,,
$$
...
Following \cite{difonzo2023}, we use a matrix organization of residuals that is similar to the one seen for base forecasts in \autoref{ssec:oct}. Specifically, let $N$ be the total number of observations for the most temporally aggregate time series. Then, the $N_k$-vector of residuals with $N_k = Nm/k$ for the temporal aggregation $k$ and the series $i$,
$$
...
\begin{equation}\label{eq:Evetki}
	\Evet_i^{[k]} = \begin{bmatrix}
		...
		\vdots                            & \vdots                            &       & \vdots                  \\
		e_{i,1,N_k - \frac{m}{k} + 1}^{[k]} & e_{i,2,N_k - \frac{m}{k} + 2}^{[k]} & \dots & e_{i,N_k,N_k}^{[k]}         \\
	\end{bmatrix}.
\end{equation}
